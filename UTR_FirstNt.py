...
Principe : 
    ...
    7- De plus, on veut que la séquence commence par un C
'''
...
def find_STOP_codon(sequence):
    ...
    # on retourne la position du codon STOP, sa séquence, et aussi les 4 premiers nucléotides de la séquence complète
    for i in range (30,len(sequence),3):
        if sequence[i:i+3] in ('TAA','TAG','TGA'):
            return i+1, str(sequence[i:i+3]), str(sequence[0:4])
    return 0, '', str(sequence[0:4])
...
for ligne in fi:
    ...
    if(old_gene!=gene):
        ...
        stop_codon, codon, first_nuc = find_STOP_codon(UTR3)
        if(stop_codon):
            ...
            if(direction=='+'):
                fo.write('\t'.join([chrom, str(start), str(start+stop_codon), gene, direction, codon, first_nuc])+'\n')
            else:
                fo.write('\t'.join([chrom, str(end-stop_codon), str(end), gene, direction, codon, first_nuc])+'\n')
            skip_gene = True
        else:
            # Pas de codon STOP dans cet exon, donc on écrit l'exon et on passe au suivant
            fo.write('\t'.join([chrom, str(start), str(end), gene, direction, codon, first_nuc])+'\n')
